...
# Vehicle corridor is future-relevant state and must survive beam snapshots.
rep('src/core/sim.hpp',
'''  float rotation = 0;\n  float speed = phys::SPEEDS[1];\n  int32_t frame = 0;''',
'''  float rotation = 0;\n  float speed = phys::SPEEDS[1];\n  float flightFloor = 0.0f;\n  float flightCeiling = 300.0f;\n  int32_t frame = 0;''')

# Include it in beam deduplication.
rep('src/search/beam.cpp',
'''  mix(static_cast<uint64_t>(static_cast<int64_t>(std::lround(s.vy * 64.0f)) + 1000000));\n  mix(static_cast<uint64_t>(s.mode));''',
...
# Semantic deterministic state equality.
